...
{
    private readonly static Regex _releaseRegex = new("<strong>Version (?'versionName'.{4}) \\(OS build (?'versionBuild'\\d+)\\)<\\/strong>(?:(?:\\n) - (?'isEoL'End of servicing)|)(?:(?s).+?)<table.+?>\n<tr>(?:\\s*<th>.+?<\\/th>){4}\\n\\s*<\\/tr>(?'tableData'(?s).+?)\\n<\\/table>");

    private readonly static Regex _releaseTableRegex = new("<tr>\\n<td>.+?<\\/td>\\n<td>(?'releaseDate'\\d{4}-\\d{2}-\\d{2})<\\/td>\n<td>(?'buildNumber'.+?)<\\/td>\\n<td>(?:<a href=\"(?'supportArticleUrl'.+?)\".+?>(?'kbArticleId'.+?)<\\/a>|)<\\/td>\\n<\\/tr>");
